...
void assign_euclidean_crowding_distance_list (population *pop, list *lst, int front_size)
{
	...
	assign_euclidean_crowding_distance (pop, dist, obj_array, front_size);
	free (dist);
	for (i=0; i<nobj; i++)
	{
		free (obj_array[i]);
...
void assign_euclidean_crowding_distance (population *pop, int *dist, int **obj_array, int front_size)
{
	...
	for (i=0; i<nobj; i++)
	{
		for (j=1; j<front_size-1; j++)
		{
			if (pop->ind[obj_array[i][j]].crowd_dist != INF)
			{
				...
				{
					pop->ind[obj_array[i][j]].crowd_dist += pow((pop->ind[obj_array[i][j+1]].obj[i] - pop->ind[obj_array[i][j-1]].obj[i])/(pop->ind[obj_array[i][front_size-1]].obj[i] - pop->ind[obj_array[i][0]].obj[i]), 2.0);
				}
			}
		}
	}
	for (j=0; j<front_size; j++)
	{
		...
		{
			pop->ind[dist[j]].crowd_dist = pow(pop->ind[dist[j]].crowd_dist, 0.5);
