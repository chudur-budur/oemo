...
void fill_nondominated_sort_euclidean (population *mixed_pop, population *new_pop)
{
	...
	do
	{
		...
		if ( (archieve_size+front_size) <= popsize)
		{
			...
			while (temp2 != NULL);
			assign_euclidean_crowding_distance_indices (new_pop, j, i-1);
			rank+=1;
		}
		else
		{
			euclidean_crowding_fill (mixed_pop, new_pop, i, front_size, elite);
...
void euclidean_crowding_fill (population *mixed_pop, population *new_pop, int count, int front_size, list *elite)
{
	int *dist;
	list *temp;
	int i, j;
	assign_euclidean_crowding_distance_list (mixed_pop, elite->child, front_size);
